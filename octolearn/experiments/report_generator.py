...
class _HeaderFooterCanvas(canvas.Canvas):
    """Canvas subclass that draws headers and footers on each page."""

    def __init__(self, *args, logo_path=None, title="OctoLearn Intelligence Report", **kwargs):
    ...
    def _draw_header_footer(self, page_num, total_pages):
        """Draw header bar and footer on every page except the first (cover)."""
        if page_num == 1:
            return
        ...
        self.setFillColor(ReportColors.TEXT_ON_DARK)
        self.drawString(45, h - 24, self._title)

        # Date in header
        self.setFont('Helvetica', 8)
        self.drawRightString(w - 15, h - 24, datetime.now().strftime("%B %d, %Y"))

        # --- Footer ---
        self.setFillColor(ReportColors.DIVIDER)
        self.line(50, 30, w - 50, 30)
        self.setFont('Helvetica', 8)
        self.setFillColor(ReportColors.TEXT_CAPTION)
        self.drawString(50, 18, "Generated by OctoLearn AutoML")
        self.drawRightString(w - 50, 18, f"Page {page_num} of {total_pages}")
...
class ReportGenerator:
    ...
    def _add_feature_intelligence(self, story):
        ...
        if self.shap_path and os.path.exists(self.shap_path):
            ...
            try:
                shap_img = Image(self.shap_path, width=5.2 * inch, height=3.5 * inch)
    ...
    def _add_visual_insights(self, story):
        ...
        if self.heatmap_plot and os.path.exists(self.heatmap_plot):
            ...
            try:
                hm_img = Image(self.heatmap_plot, width=5.5 * inch, height=4 * inch)
    ...
    def generate(self, filename: Optional[str] = None) -> str:
        ...
        try:
            # Use custom canvas for headers/footers
            def canvas_maker(fn, **kwargs):
                return _HeaderFooterCanvas(
                    fn, logo_path=self.logo_path, title=self.title, **kwargs
                )
